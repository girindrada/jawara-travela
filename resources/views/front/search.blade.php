...
@section('content')
    <section id="content" class="max-w-[640px] w-full mx-auto bg-[#F9F2EF] min-h-screen flex flex-col gap-8 pb-[120px]">
        <nav class="mt-8 px-4 w-full flex items-center justify-between">
            ...
            <p class="text-center m-auto font-semibold">Search</p>
            <div class="w-12"></div>
        </nav>
        ...
        <div class="mx-4 flex flex-col gap-3 bg-white p-[16px_24px] rounded-[22px]">
            <form method="GET" action="{{ route('front.search') }}" class="flex w-full gap-2">
                <div
                    class="flex items-center gap-3 p-[16px_12px] border border-[#BFBFBF] rounded-xl focus-within:border-[#4D73FF] transition-all duration-300 w-full">
                ...
                <button type="submit" class="px-4 bg-[#4D73FF] text-white rounded-xl text-sm font-semibold shrink-0">
                    Search
                </button>
            </form>

            <div id="explore" class="flex flex-col px-4 gap-3">
                <h2 class="font-semibold">Destination To Explore</h2>
