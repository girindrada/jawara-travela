    ...
    <section id="content" class="max-w-[640px] w-full mx-auto bg-[#F9F2EF] min-h-screen flex flex-col gap-8 pb-[120px]">
        ...
        @guest
          <div class="flex items-center gap-3">
            ...
            <div class="flex flex-col gap-1">
              <p class="text-xs tracking-035">Hey, There!</p>
              <p class="font-semibold">Let's Travel Together</p>
            </div>
          </div>
        @endguest
            ...
            @forelse ($categories as $category)
                <a href="{{ route('front.category', $category->slug) }}" class="group px-2 first-of-type:pl-4 last-of-type:pr-4">
                    <div class="p-3 flex items-center gap-2 rounded-[10px] border border-[#4D73FF] group-hover:bg-[#4D73FF] transition-all duration-300">
                        ...
                        <span class="text-sm tracking-[0.35px] text-[#4D73FF] group-hover:text-white transition-all duration-300">
                            {{ $category->name }}
                        </span>
                    </div>
                </a>
            @empty
                <a href="{{ route('front.index') }}" class="group px-2 first-of-type:pl-4 last-of-type:pr-4">
                    <div class="p-3 flex items-center gap-2 rounded-[10px] border border-[#4D73FF] group-hover:bg-[#4D73FF] transition-all duration-300">
                        <div class="w-6 h-6 flex shrink-0">
                        <img src="{{ asset('assets/icons/umbrella.svg') }}" alt="icon">
                        </div>
                        <span class="text-sm tracking-[0.35px] text-[#4D73FF] group-hover:text-white transition-all duration-300">Oops..</span>
                    </div>
                </a>
            @endforelse
        ...
        <div id="recommendations" class="flex flex-col gap-3">
          ...
          <div class="main-carousel card-container">
            ...
            @forelse ($packageTours as $packageTour)
                <a href="{{ route('front.details', $packageTour->slug) }}" class="group px-2 first-of-type:pl-4 last-of-type:pr-4">
                    <div class="w-[288px] p-4 flex flex-col gap-3 rounded-[26px] bg-white shadow-[6px_8px_20px_0_#00000008]">
                        ...
                        <img src="{{ Storage::url($packageTour->thumbnail)}}" class="w-full h-full object-cover" alt="thumbnails">
                        </div>
                        ...
                        <div class="flex flex-col gap-1 text-right">
                            ...
                            <span class="text-darkGrey">/{{ $packageTour->days}} days</span>
                            </p>
            ...
            @empty
                <p>data rekomendasi paket tour kosong</p>
            @endforelse
        ...
        <div id="explore" class="flex flex-col px-4 gap-3">
          ...
          @forelse ($packageTours as $tour)
            <a href="{{ route('front.details', $packageTour->slug) }}" class="card">
