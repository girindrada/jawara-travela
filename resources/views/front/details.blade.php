        ...
        <div id="image-details" class="px-4 flex flex-col gap-3">
          ...
          <div class="grid grid-cols-4 gap-4 w-fit mx-auto">
            ...
            @foreach($latestPhotos as $latestPhoto)
                <a href="{{ Storage::url($latestPhoto->photo)}}" class="thumbnail-option w-[74px] h-[74px] flex shrink-0 rounded-xl border-2 overflow-hidden mx-auto opacity-50">
                    <img src="{{ Storage::url($latestPhoto->photo)}}" class="w-full h-full object-cover object-center" alt="thumbnail">
                </a>
            @endforeach
